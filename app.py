...
import logging
import traceback
from utils.mongo_config import get_mongodb_connection, get_collection
...
def init_db_routes(app):
    """
    Initialize database management routes
    """
    ...
    def database_view():
        ...
        """
        try:
            # Get MongoDB client - use your existing function
            mongo_client = get_mongodb_connection()
            if not mongo_client:
                logger.error("Failed to get MongoDB connection")
                return jsonify({"error": "Error connecting to database. Please check if database is accessible."}), 500
  ...
  "field": "value"
    ...
    def collection_management(db_name, collection_name):
        ...
        """
        try:
            # Use your existing function for consistency
            collection = get_collection(db_name, collection_name)
            if not collection:
                return jsonify({"error": "Database or collection not found"}), 404
    ...
    @app.route('/database/debug', methods=['GET'])
    def database_debug():
        ...
        try:
            ...
            if not mongo_client:
                return jsonify({
                    "status": "error",
                    "message": "get_mongodb_connection() returned None"
                }), 500
            ...
            try:
                ...
                return jsonify({
                    "status": "error",
                    "message": f"MongoDB connection failed during database listing: {str(e)}"
                }), 500
